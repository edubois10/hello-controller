...
# bit.admin.ch/controller-bundle:$VERSION and bit.admin.ch/controller-catalog:$VERSION.
IMAGE_TAG_BASE ?= quay.io/edubois10/drs-vm-group/controller
...
.PHONY: envtestVersion
envtestVersion:
	@printf ${ENVTEST_K8S_VERSION}

.PHONY: repository
repository:
	@printf ${IMAGE_TAG_BASE}
...
.PHONY: test-offline
test-offline: manifests generate fmt vet envtest ## Run tests.
...
.PHONY: dry-run
dry-run: manifests generate fmt vet
	go run ./main.go --dry-run	

.PHONY: docker-build
docker-build: ## Build docker image with the manager.
	buildah build $(ARGS) -t ${IMG} -t ${IMG_LATEST} .

.PHONY: docker-push
docker-push: ## Push docker image with the manager.
	buildah push $(ARGS) ${IMG}
